...
pub const Ingredient = struct { id: u16, meta: ?u4 = null };
...
const recipes = [_]Recipe{
    .{
        .width = 1,
        .height = 1,
        .pattern = .{ .{ .id = world.block.log }, null, null, null },
        .output_id = world.block.planks,
        .output_count = 4,
    },
    .{
        .width = 1,
        .height = 2,
        .pattern = .{ .{ .id = world.block.planks }, .{ .id = world.block.planks }, null, null },
        .output_id = world.item.stick,
        .output_count = 4,
    },
    .{
        .width = 2,
        .height = 2,
        .pattern = .{
            .{ .id = world.item.clay_ball },
            .{ .id = world.item.clay_ball },
            .{ .id = world.item.clay_ball },
            .{ .id = world.item.clay_ball },
        },
        .output_id = world.block.clay,
        .output_count = 1,
    },
};
...
fn matchesAt(grid: [grid_size * grid_size]?Inventory.ItemStack, recipe: Recipe, offset_x: u8, offset_y: u8) bool {
    for (0..grid_size) |y| {
        for (0..grid_size) |x| {
            ...
            if (have.?.id != want.?.id) return false;
            if (want.?.meta) |m| {
...
test "log crafts into planks in any grid cell" {
    ...
    grid[3] = .{ .id = world.block.log, .count = 1, .meta = 2 };
    const result = findMatch(grid).?;
    try std.testing.expectEqual(world.block.planks, result.id);
    try std.testing.expectEqual(@as(u8, 4), result.count);
}

test "two stacked planks craft into sticks" {
    var grid: [4]?Inventory.ItemStack = @splat(null);
    grid[0] = .{ .id = world.block.planks, .count = 5 };
    grid[2] = .{ .id = world.block.planks, .count = 5 };
    const result = findMatch(grid).?;
    try std.testing.expectEqual(world.item.stick, result.id);
    try std.testing.expectEqual(@as(u8, 4), result.count);
}
...
test "planks side by side do not match the stick recipe" {
    var grid: [4]?Inventory.ItemStack = @splat(null);
    grid[0] = .{ .id = world.block.planks, .count = 5 };
    grid[1] = .{ .id = world.block.planks, .count = 5 };
    try std.testing.expect(findMatch(grid) == null);
}

test "four clay balls fill the grid to craft a clay block" {
    var grid: [4]?Inventory.ItemStack = @splat(null);
    for (&grid) |*slot| slot.* = .{ .id = world.item.clay_ball, .count = 1 };
    const result = findMatch(grid).?;
    try std.testing.expectEqual(world.block.clay, result.id);
    try std.testing.expectEqual(@as(u8, 1), result.count);
}

test "three clay balls are not enough to fill the grid" {
    var grid: [4]?Inventory.ItemStack = @splat(null);
    grid[0] = .{ .id = world.item.clay_ball, .count = 1 };
    grid[1] = .{ .id = world.item.clay_ball, .count = 1 };
    grid[2] = .{ .id = world.item.clay_ball, .count = 1 };
    try std.testing.expect(findMatch(grid) == null);
}

test "an unrelated extra item in the grid blocks the match" {
    var grid: [4]?Inventory.ItemStack = @splat(null);
    grid[0] = .{ .id = world.block.log, .count = 1 };
    grid[1] = .{ .id = world.block.dirt, .count = 1 };
    try std.testing.expect(findMatch(grid) == null);
}

test "consume removes one item from every filled grid slot" {
    var grid: [4]?Inventory.ItemStack = @splat(null);
    grid[0] = .{ .id = world.block.planks, .count = 1 };
    grid[2] = .{ .id = world.block.planks, .count = 2 };
    consume(&grid);
