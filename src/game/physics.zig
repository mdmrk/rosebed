...
fn collidingBoxes(chunk: *const world.Chunk, query: math.AABB, out: *[max_colliding_boxes]math.AABB) usize {
    const min_x = std.math.clamp(math.util.floorDouble(query.min_x), 0, world.constants.chunk_width - 1);
    const max_x = std.math.clamp(math.util.floorDouble(query.max_x), 0, world.constants.chunk_width - 1);
    ...
    const min_z = std.math.clamp(math.util.floorDouble(query.min_z), 0, world.constants.chunk_width - 1);
    const max_z = std.math.clamp(math.util.floorDouble(query.max_z), 0, world.constants.chunk_width - 1);
...
pub fn moveEntity(chunk: *const world.Chunk, aabb: math.AABB, dx: f64, dy: f64, dz: f64) MoveResult {
    var box_buf: [max_colliding_boxes]math.AABB = undefined;
    const broad = aabb.addCoord(dx, dy, dz);
    const count = collidingBoxes(chunk, broad, &box_buf);
...
fn testChunkWithFloor(floor_top_y: u32) world.Chunk {
...
test "falling onto a floor stops the vertical offset at the surface" {
    const chunk = testChunkWithFloor(1);
    const aabb = math.AABB.init(0, 1.1, 0, 0.6, 2.9, 0.6);
    const result = moveEntity(&chunk, aabb, 0, -0.5, 0);
...
test "walking into a wall clamps the horizontal offset at the surface" {
    var chunk = world.Chunk.init(0, 0);
    chunk.setBlockId(2, 0, 0, world.block.stone);
    const aabb = math.AABB.init(1.2, 0, -0.3, 1.8, 1.8, 0.3);
    const result = moveEntity(&chunk, aabb, 0.5, 0, 0);
...
test "open air applies the full requested movement" {
    ...
    const result = moveEntity(&chunk, aabb, 1.0, -1.0, 1.0);
