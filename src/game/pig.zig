...
pub const eye_height: f64 = height * 0.85;
...
pub fn isAlive(self: Pig) bool {
    return !self.dead and self.health > 0;
}

fn clearPath(self: *Pig, gpa: std.mem.Allocator) void {
    if (self.path) |*path| path.deinit(gpa);
    self.path = null;
}

fn pathMob(self: Pig) world.pathfinder.Mob {
...
fn eyePosition(self: Pig) math.Vec3 {
    return .{
...
fn distanceSquaredTo(self: Pig, position: math.Vec3) f64 {
...
fn isInsideOpaqueBlock(self: Pig, world_map: *const world.World) bool {
    for (0..8) |corner| {
        ...
        const y = math.util.floorDouble(self.base.position.y + eye_height + dy);
...
pub fn canSpawnHere(self: Pig, world_map: *const world.World) bool {
    const box = self.base.boundingBox();
...
pub fn hurt(self: *Pig, amount: i32, source: ?math.Vec3, rand: *world.JavaRandom) bool {
    self.entity_age = 0;
    ...
    if (self.health <= 0) self.dropFewItems(rand);
    return true;
}

fn knockBack(self: *Pig, dx: f64, dz: f64) void {
...
fn dropFewItems(self: *Pig, rand: *world.JavaRandom) void {
    self.pending_drops = @intCast(rand.nextIntBound(3));
    self.drops_cooked = self.fire > 0;
}

pub const Drops = struct { count: u8, id: world.Item };
...
fn fall(self: *Pig, distance: f32, rand: *world.JavaRandom) void {
    const damage: i32 = @intFromFloat(@ceil(distance - safe_fall_distance));
    if (damage > 0) _ = self.hurt(damage, null, rand);
}

fn updateFallState(self: *Pig, dy: f64, rand: *world.JavaRandom) void {
...
fn moveFlying(self: *Pig, strafe: f32, forward: f32, acceleration: f32) void {
...
fn moveWithHeading(self: *Pig, world_map: *const world.World, strafe: f32, forward: f32, rand: *world.JavaRandom) void {
...
fn faceEntity(self: *Pig, target: math.Vec3, target_eye_height: f64, yaw_speed: f32, pitch_speed: f32) void {
    const dx = target.x - self.base.position.x;
    const dz = target.z - self.base.position.z;
    const dy = (self.base.position.y + eye_height) - (target.y + target_eye_height);
...
fn despawnCheck(self: *Pig, player: ?PlayerView, rand: *world.JavaRandom) void {
...
fn idleActionState(self: *Pig, player: ?PlayerView, rand: *world.JavaRandom) void {
    self.entity_age += 1;
...
fn findWanderPath(self: *Pig, gpa: std.mem.Allocator, world_map: *const world.World, rand: *world.JavaRandom) !void {
...
fn followPath(self: *Pig, gpa: std.mem.Allocator, rand: *world.JavaRandom) void {
    const feet_y: f64 = @floatFromInt(math.util.floorDouble(self.base.boundingBox().min_y + 0.5));
    const reach = width * 2.0;
...
fn updateFireAndWater(self: *Pig, world_map: *const world.World, rand: *world.JavaRandom) void {
...
fn updateBreathing(self: *Pig, world_map: *const world.World, rand: *world.JavaRandom) void {
...
fn updateRenderYaw(self: *Pig) void {
    const dx = self.base.position.x - self.base.prev_position.x;
    const dz = self.base.position.z - self.base.prev_position.z;
...
pub fn limbSwingPhase(self: Pig, partial_ticks: f32) f32 {
    return self.limb_swing - self.limb_swing_amount * (1.0 - partial_ticks);
}

pub fn deathTilt(self: Pig, partial_ticks: f32) f32 {
...
test "gravity pulls a spawned pig down" {
    ...
    try pig.tick(gpa, &w, null, &rand);
    try std.testing.expectApproxEqAbs(@as(f64, -0.0784), pig.base.motion.y, 1.0e-9);

    try pig.tick(gpa, &w, null, &rand);
    try std.testing.expect(pig.base.position.y < 5.0);
}

test "a pig on grass eventually picks a path and walks it" {
    const gpa = std.testing.allocator;
    var w = try grassWorld(gpa);
    defer w.deinit();

    var rand = world.JavaRandom.init(7);
    var pig = Pig.spawn(math.Vec3.init(8.5, 1, 8.5));
    defer pig.deinit(gpa);
    ...
    for (0..400) |_| {
        try pig.tick(gpa, &w, null, &rand);
        if (pig.path != null) walked = true;
    }

    try std.testing.expect(walked);
    try std.testing.expect(pig.base.position.x != 8.5 or pig.base.position.z != 8.5);
}

test "a wandering pig walks along the surface rather than sinking or floating" {
    const gpa = std.testing.allocator;
    var w = try grassWorld(gpa);
    defer w.deinit();
    ...
    for (0..200) |_| {
        ...
        try std.testing.expectApproxEqAbs(@as(f64, 1.0), pig.base.position.y, 1.0e-6);
    }
}
...
test "damage inside the invulnerability window only counts what exceeds the last hit" {
    ...
    try std.testing.expect(pig.hurt(3, null, &rand));
    try std.testing.expectEqual(@as(i32, 7), pig.health);
    try std.testing.expectEqual(@as(i32, 10), pig.hurt_time);

    try std.testing.expect(!pig.hurt(2, null, &rand));
    try std.testing.expectEqual(@as(i32, 7), pig.health);

    try std.testing.expect(pig.hurt(5, null, &rand));
    try std.testing.expectEqual(@as(i32, 5), pig.health);
}

test "a hit from a known source knocks the pig away from it" {
    var rand = world.JavaRandom.init(0);
    var pig = Pig.spawn(math.Vec3.init(8, 1, 8));

    _ = pig.hurt(1, math.Vec3.init(6, 1, 8), &rand);

    try std.testing.expect(pig.base.motion.x > 0.0);
    try std.testing.expectApproxEqAbs(@as(f64, 0.4), pig.base.motion.y, 1.0e-9);
}
...
test "a dying pig drops nought to two porkchops" {
    ...
    for (0..20) |seed| {
        ...
        _ = pig.hurt(max_health, null, &rand);
        try std.testing.expect(!pig.isAlive());
...
test "a killed pig lingers for twenty ticks before it is removed" {
    const gpa = std.testing.allocator;
    var w = try testing_world.flatWorld(gpa, 1);
    defer w.deinit();
    ...
    for (0..death_ticks) |_| {
        try pig.tick(gpa, &w, null, &rand);
        try std.testing.expect(!pig.dead);
    }
...
test "a long fall hurts by the distance beyond three blocks" {
    ...
    var rand = world.JavaRandom.init(0);
    var pig = Pig.spawn(math.Vec3.init(8.5, 20, 8.5));
    defer pig.deinit(gpa);

    for (0..100) |_| {
        try pig.tick(gpa, &w, null, &rand);
        if (pig.base.on_ground) break;
    }

    try std.testing.expect(pig.base.on_ground);
    try std.testing.expect(pig.health < max_health);
    try std.testing.expectApproxEqAbs(@as(f32, 0.0), pig.fall_distance, 1.0e-6);
}
...
test "a short drop is walked off without damage" {
    ...
    var rand = world.JavaRandom.init(0);
    var pig = Pig.spawn(math.Vec3.init(8.5, 3.5, 8.5));
    defer pig.deinit(gpa);
    ...
    try std.testing.expect(pig.base.on_ground);
    try std.testing.expectEqual(max_health, pig.health);
}

test "a pig held under water drowns once its air runs out" {
    const gpa = std.testing.allocator;
    ...
    for (0..max_air) |_| try pig.tick(gpa, &w, null, &rand);
    try std.testing.expectEqual(max_health, pig.health);
    try std.testing.expectEqual(@as(i32, 0), pig.air);

    for (0..20) |_| try pig.tick(gpa, &w, null, &rand);
    try std.testing.expectEqual(max_health - drown_damage, pig.health);
}

test "a pig in open water swims up and never drowns" {
    const gpa = std.testing.allocator;
    var w = try testing_world.flatWorld(gpa, 1);
    defer w.deinit();
    ...
    for (0..max_air + 100) |_| try pig.tick(gpa, &w, null, &rand);

    try std.testing.expectEqual(max_health, pig.health);
    try std.testing.expect(pig.base.position.y > 4.0);
}

test "a pig buried in stone suffocates" {
    const gpa = std.testing.allocator;
    var w = try testing_world.flatWorld(gpa, 6);
    defer w.deinit();

    var rand = world.JavaRandom.init(0);
    var pig = Pig.spawn(math.Vec3.init(8.5, 3, 8.5));
    defer pig.deinit(gpa);

    try pig.tick(gpa, &w, null, &rand);
    try std.testing.expectEqual(max_health - suffocation_damage, pig.health);
}

test "a pig standing in lava catches fire and burns" {
    const gpa = std.testing.allocator;
    var w = try testing_world.flatWorld(gpa, 1);
    defer w.deinit();
    ...
    try pig.tick(gpa, &w, null, &rand);
    try std.testing.expectEqual(max_health - lava_damage, pig.health);
    try std.testing.expect(pig.fire > 0);
}

test "a pig far from the player despawns" {
    const gpa = std.testing.allocator;
    var w = try testing_world.flatWorld(gpa, 1);
    defer w.deinit();
    ...
    for (0..200) |_| {
        try pig.tick(gpa, &w, far_away, &rand);
        if (pig.dead) break;
    }
...
test "a pig near the player is kept alive however old it gets" {
    const gpa = std.testing.allocator;
    var w = try grassWorld(gpa);
    defer w.deinit();
    ...
    for (0..600) |_| try pig.tick(gpa, &w, nearby, &rand);

    try std.testing.expect(!pig.dead);
}

test "an idle pig turns to watch a player who walks up to it" {
    const gpa = std.testing.allocator;
    var w = try testing_world.flatWorld(gpa, 1);
    defer w.deinit();
    ...
    var watched = false;
    for (0..300) |_| {
        try pig.tick(gpa, &w, nearby, &rand);
        if (pig.looking_at_player) watched = true;
    }

    try std.testing.expect(watched);
}

test "grass is the pig's preferred ground to wander onto" {
    const gpa = std.testing.allocator;
    var w = try testing_world.flatWorld(gpa, 1);
    defer w.deinit();
...
test "a pig only spawns on lit grass" {
    const gpa = std.testing.allocator;
    var w = try grassWorld(gpa);
    defer w.deinit();

    const on_grass = Pig.spawn(math.Vec3.init(8.5, 1, 8.5));
...
test "the limb swing amount saturates at one however hard the pig is hit" {
    var rand = world.JavaRandom.init(0);
    var pig = Pig.spawn(math.Vec3.init(8, 1, 8));

    _ = pig.hurt(1, null, &rand);
    try std.testing.expectApproxEqAbs(@as(f32, 1.5), pig.limb_swing_amount, 1.0e-6);
    try std.testing.expectApproxEqAbs(@as(f32, 1.0), pig.limbSwingAmount(1.0), 1.0e-6);
}

test "the death tilt rolls the corpse onto its side over the death animation" {
    var pig = Pig.spawn(math.Vec3.init(8, 1, 8));

    try std.testing.expectApproxEqAbs(@as(f32, 0.0), pig.deathTilt(0), 1.0e-6);

    pig.death_time = 1;
    try std.testing.expect(pig.deathTilt(1.0) > 0.0);

    pig.death_time = death_ticks;
    try std.testing.expectApproxEqAbs(@as(f32, 90.0), pig.deathTilt(1.0), 1.0e-6);
}

test "a pig walks the way it is facing" {
    var north = Pig.spawn(math.Vec3.init(8, 1, 8));
    north.yaw = 0;
    ...
    var west = Pig.spawn(math.Vec3.init(8, 1, 8));
    west.yaw = 90;
...
test "the body turns to follow the head rather than snapping to it" {
    ...
    try std.testing.expect(pig.render_yaw > 0.0);
    try std.testing.expect(pig.render_yaw <= 90.0);
    try std.testing.expect(@abs(pig.headYaw(1.0)) <= 75.0);
}
