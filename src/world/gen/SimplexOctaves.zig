...
pub const Scale = SimplexNoise.BatchScale;

const SimplexOctaves = @This();

generators: []SimplexNoise,

pub fn init(gpa: std.mem.Allocator, rand: *JavaRandom, octaves: usize) !SimplexOctaves {
    const generators = try gpa.alloc(SimplexNoise, octaves);
    for (generators) |*g| g.* = SimplexNoise.init(rand);
    return .{ .generators = generators };
}

pub fn deinit(self: SimplexOctaves, gpa: std.mem.Allocator) void {
...
test "generateDefault matches a real Minecraft SimplexOctaves(seed 777, 4 octaves)" {
    const gpa = std.testing.allocator;
    var rand = JavaRandom.init(777);
    const gen = try SimplexOctaves.init(gpa, &rand, 4);
