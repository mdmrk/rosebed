...
pub const Scale = PerlinNoise.BatchScale;

const PerlinOctaves = @This();

generators: []PerlinNoise,

pub fn init(gpa: std.mem.Allocator, rand: *JavaRandom, octaves: usize) !PerlinOctaves {
    const generators = try gpa.alloc(PerlinNoise, octaves);
    for (generators) |*g| g.* = PerlinNoise.init(rand);
    return .{ .generators = generators };
}

pub fn deinit(self: PerlinOctaves, gpa: std.mem.Allocator) void {
...
pub fn samplePoint2D(self: PerlinOctaves, x: f64, z: f64) f64 {
    var total: f64 = 0.0;
...
test "matches a real Minecraft PerlinOctaves(seed 999, 4 octaves) 2D grid" {
    const gpa = std.testing.allocator;
    var rand = JavaRandom.init(999);
    const gen = try PerlinOctaves.init(gpa, &rand, 4);
...
test "matches a real Minecraft PerlinOctaves(seed 999, 4 octaves) 3D grid" {
    const gpa = std.testing.allocator;
    var rand = JavaRandom.init(999);
    const gen = try PerlinOctaves.init(gpa, &rand, 4);
    defer gen.deinit(gpa);
